...
export const generateProjectHTML = (project) => {
  const { id, title, category = [], description, type, technologies = "", image, links } = project;

  const categoryString = category.join(" ");
  const hasLink = links?.preview || links?.website || links?.ios || links?.android || links?.github;
  ...
  const imageHTML = hasLink
    ? `<a href="${links.preview || links.website || links.ios || links.android || links.github || '#'}">
...
export const generateSideworkHTML = (sidework) => {
  ...
  const categoriesHTML = categories.length > 0
    ? `<div class="blog-meta">${categories.map(cat => `<p class="blog-category">${cat}</p>`).join('<span class="dot"></span>')}</div>`
    : "";
